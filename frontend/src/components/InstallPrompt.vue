<template>
	...
	<Dialog v-model="showDialog">
		<template #body-title>
			<h2 class="text-lg font-bold">{{ __("Install Dhruvanta HRMS") }} </h2>
		</template>
		<template #body-content>
			<p>{{ __("Get Dhruvanta HRMS on your device for easy access & a better experience!") }} </p>
			<p class="mt-2 text-sm text-gray-500">{{ __("Powered by Frappe HR") }}</p>
		</template>
		<template #actions>
			...
			>
				...
				<div class="text-xs text-gray-800 px-3">
					<span class="flex flex-col gap-2">
						<span>
							{{ __("Get Dhruvanta HRMS on your iPhone for easy access & a better experience") }}
						</span>
