...
Status: **CONTRACT LOCKED; AUTH GUARD SOURCE-WIRED; HEALTH, EMPLOYEE READ, LEAVE LIST, ATTENDANCE LIST, ATTENDANCE CHECKINS, ROSTER EVENTS, PAYROLL SLIPS, AND AUDIT EVENTS SOURCE-WIRED** for
`/api/v1/service/hrms/*`.
...
directory, employee detail reads, leave list reads, attendance list reads,
attendance check-in writes, roster event reads, payroll slip reads, and audit event reads, while the
